import messages from "./SpamMessages";
import dictionary from "./SpamDictionary";
...
class Spam {
    ...
    get() {
        let full = [
            ...
            `{There is|There's} {definitely|certainly} {a lot to|a great deal to} {know about|learn about|find out about} this {subject|topic|issue}. {I like|I love|I really like} {all the|all of the} points {you made|you've made|you have made}.`,
            ...
            `I {simply|just} {could not|couldn't} {leave|depart|go away} your {site|web site|website} {prior to|before} suggesting that I {really|extremely|actually} {enjoyed|loved} {the standard|the usual} {information|info} {a person|an individual} {supply|provide} {for your|on your|in your|to your} {visitors|guests}? Is {going to|gonna} be {back|again} {frequently|regularly|incessantly|steadily|ceaselessly|often|continuously} {in order to|to} {check up on|check out|inspect|investigate cross-check} new posts|{I wanted|I needed|I want to|I need to} to thank you for this {great|excellent|fantastic|wonderful|good|very good} read!! I {definitely|certainly|absolutely} {enjoyed|loved} every {little bit of|bit of} it. {I have|I've got|I have got} you {bookmarked|book marked|book-marked|saved as a favorite} {to check out|to look at} new {stuff you|things you} post…|{Hi|Hello|Hi there|What's up}, just wanted to {mention|say|tell you}, I {enjoyed|liked|loved} this {article|post|blog post}. It was {inspiring|funny|practical|helpful}. Keep on posting!`,
            `I {leave|drop|write|create} a {comment|response} {each time|when|whenever} I {appreciate|like|especially enjoy} a {post|article} on a {site|{blog|website}|site|website} or {I have|if I have} something to {add|contribute|valuable to contribute} {to the discussion|to the conversation}. {It is|Usually it is|Usually it's|It's} {a result of|triggered by|caused by} the {passion|fire|sincerness} {communicated|displayed} in the {post|article} I {read|looked at|browsed}. And {on|after} this {post|article} I {was|was actually} {moved|excited} enough to {drop|leave|write|create|post} {a thought|a comment|a response} {:-P|:)|;)|;-)|:-)} I {do have|actually do have} {some|a few|a couple of} questions for you {if you don't|do not|usually do not|tend not to} mind. {Is it|Could it be} {just|only|simply} me or does it seem like {some|a few} of {the|these} {comments|responses|remarks} {look|appear|come across} {like they are|as if they are|like} {coming from|written by|left by} brain dead {people|visitors|folks|individuals}? :-P And, if you are {posting|writing} {on|at} {other|additional} {sites|social sites|online sites|online social sites|places}, {I'd|I would} like to {follow|keep up with} {anything|everything} {new|fresh} you have to post. {Could|Would} you {list|make a list} {all|every one|the complete urls} of {your|all your} {social|communal|community|public|shared} {pages|sites} like your {twitter feed, Facebook page or linkedin profile|linkedin profile, Facebook page or twitter feed|Facebook page, twitter feed, or linkedin profile}?`,
        ...
        let newFull = [];
        for (let j in full) {
            let synonyms = this.findIn(full[j]), words = [], comment;

            for (let i in synonyms) {
                let _ = synonyms[i].split("|");
                words[`{${synonyms[i]}}`] = `synonym:${_[0].replace(" ", "-")}`;
            }

            comment = this.replace(full[j], words);
            newFull.push(
                comment.split(" ")
            );
        }
    ...
    make() {
        let message = this.arrayRand(this.messages());

        for (let i in message) {
            if (message[i].includes(':')) {
                let split = message[i].split(':'), func = split[0], arg = split[1];
                arg = arg.replace("-", " ");
                if (arg.includes('|')) {
                    arg = arg.split('|');
                    message[i] = this.dictionary[func](arg[0], arg[1]);
                } else {
                    message[i] = this.dictionary[func](arg);
                }
    ...
    getWord(synonym) {
        let words = synonym.split("|"),
            rand = this.rand(0, words.length - 1);

        return words[rand];
    }

    replace(string, words) {
        for (let searchvalue in words) {
            string = string.replace(searchvalue, words[searchvalue]);
        }
